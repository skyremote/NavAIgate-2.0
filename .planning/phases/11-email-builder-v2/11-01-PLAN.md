...
<task type="auto">
  <name>Task 1: Create OpenRouter AI service</name>
  <files>src/lib/openrouter.ts</files>
  <action>
    Create a service to call OpenRouter API for email generation:
    - Function to generate email content from prompt
    - Use claude-3-haiku for cost efficiency
    - Handle streaming response
    - Return structured email content (subject, body sections)

    API endpoint: https://openrouter.ai/api/v1/chat/completions
    Headers: Authorization: Bearer {key}, HTTP-Referer: https://navaigate.dev
  </action>
  <verify>Service file exists with proper TypeScript types and fetch implementation</verify>
  <done>openrouter.ts exports generateEmail function that calls OpenRouter API</done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    - AI tab: Form with context, tone, recipient → generates email
